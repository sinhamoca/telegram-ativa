...
/**
 * Migração automática para notificações de ativações
 */
function migrateNotifAtivacoes() {
  try {
    const columns = db.exec("PRAGMA table_info(usuarios)")[0]?.values || [];
    const columnNames = columns.map(col => col[1]);
    
    if (!columnNames.includes('notif_ativacoes')) {
      console.log('[DB] Migrando: adicionando notif_ativacoes...');
      db.run("ALTER TABLE usuarios ADD COLUMN notif_ativacoes TEXT DEFAULT 'desativado'");
      saveDb();
      console.log('[DB] ✅ Coluna notif_ativacoes adicionada');
    }
  } catch (e) {
    console.error('[DB] Erro na migração notif_ativacoes:', e.message);
  }
}
...
const usuarios = {
  ...
  getNotifAtivacoes(usuarioId) {
    const result = queryOne('SELECT notif_ativacoes FROM usuarios WHERE id = ?', [usuarioId]);
    return result?.notif_ativacoes || 'desativado';
  },

  setNotifAtivacoes(usuarioId, config) {
    run('UPDATE usuarios SET notif_ativacoes = ? WHERE id = ?', [config, usuarioId]);
    saveDb();
  },
