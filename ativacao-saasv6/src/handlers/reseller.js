...
const { consultar, formatarMensagem } = require('../modules/antipapateste');
...
const menuPrincipal = Markup.inlineKeyboard([
  ...
  [Markup.button.callback('📋 Consultar Papa Teste', 'res_papateste')],
...
/**
 * Menu Papa Teste - Revendedor
 */
async function handlePapaTesteMenu(ctx) {
  await ctx.answerCbQuery();
  
  await ctx.editMessageText(
    `📋 <b>Consultar Papa Teste</b>\n\n` +
    `Verifique se um número de telefone é "papa teste" ` +
    `(cliente que pega teste grátis e nunca compra).\n\n` +
    `<b>Classificações:</b>\n` +
    `🚫 Papa Teste Ativo - NÃO ATENDER\n` +
    `⛔ Papa Teste Recente - EVITAR\n` +
    `⚠️ Suspeito - CAUTELA\n` +
    `⚡ Atenção - AVALIAR\n` +
    `✅ Limpo - PODE ATENDER\n\n` +
    `Clique abaixo para consultar:`,
    {
      parse_mode: 'HTML',
      ...Markup.inlineKeyboard([
        [Markup.button.callback('🔍 Consultar Número', 'res_papateste_consultar')],
        [Markup.button.callback('🔙 Voltar', 'res_menu')]
      ])
    }
  );
}
...
/**
 * Solicita número para consulta - Revendedor
 */
async function handlePapaTesteConsultar(ctx, resellerState) {
  await ctx.answerCbQuery();
  
  resellerState.set(ctx.from.id, { step: 'papateste_numero' });
  
  await ctx.editMessageText(
    `🔍 <b>Consultar Papa Teste</b>\n\n` +
    `Digite o número de telefone que deseja consultar:\n\n` +
    ...
    `• (85) 99999-9999`,
    {
      parse_mode: 'HTML',
      ...Markup.inlineKeyboard([
        [Markup.button.callback('❌ Cancelar', 'res_papateste')]
      ])
    }
  );
}
...
/**
 * Processa número enviado - Revendedor
 * (Adicionar no switch do handleResellerText)
 */
// case 'papateste_numero': {
//   resellerState.delete(ctx.from.id);
//   
...
//       Markup.inlineKeyboard([[Markup.button.callback('🔄 Tentar novamente', 'res_papateste_consultar')]])
//     );
//     return true;
//   }
//   
//   const aguarde = await ctx.reply('⏳ Consultando número...');
//   
//   try {
//     const resultado = await consultar(numero);
//     const mensagem = formatarMensagem(resultado);
//     
//     await ctx.telegram.deleteMessage(ctx.chat.id, aguarde.message_id).catch(() => {});
//     
//     await ctx.reply(mensagem, {
//       parse_mode: 'HTML',
//       ...Markup.inlineKeyboard([
//         [Markup.button.callback('🔍 Nova Consulta', 'res_papateste_consultar')],
//         [Markup.button.callback('🔙 Voltar', 'res_papateste')]
//       ])
//     });
//   } catch (error) {
//     await ctx.telegram.deleteMessage(ctx.chat.id, aguarde.message_id).catch(() => {});
...
//       Markup.inlineKeyboard([[Markup.button.callback('🔙 Voltar', 'res_papateste')]])
//     );
//   }
//   return true;
// }
