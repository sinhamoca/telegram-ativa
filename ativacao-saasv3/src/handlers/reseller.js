...
async function handleSaldoApps(ctx) {
  ...
  */
  ...
  if (credRivolut) {
    try {
      const rivolutModule = require('../modules/rivolut');
      const resultado = await rivolutModule.getCredits(credRivolut.dados);
...
async function handleRelatorios(ctx) {
  ...
  if (topApps.length > 0) {
    ...
    topApps.slice(0, 10).forEach((app, i) => {
      ...
      topAppsTexto += `${medalhas[i]} ${nomeApp}: *${app.quantidade}*\n`;
    });
  }
  ...
  mensagem += `📅 *Ativações por Período:*\n`;
  mensagem += `├ Hoje: *${ativacoesHoje}*\n`;
  mensagem += `├ 7 dias: *${ativacoes7dias}*\n`;
  mensagem += `├ 30 dias: *${ativacoes30dias}*\n`;
  mensagem += `├ Este mês: *${ativacoesMes}*\n`;
  mensagem += `└ Total geral: *${total}*\n`;
  
  mensagem += `\n📈 *Desempenho:*\n`;
  mensagem += `├ ✅ Sucesso: *${sucesso}*\n`;
  mensagem += `├ ❌ Falhas: *${falhas}*\n`;
  mensagem += `└ 📊 Taxa: *${taxaSucesso}%*\n`;
...
async function handleResellerText(ctx) {
  ...
  switch (state.step) {
    ...
    case 'cred_rivolut_pass': {
      ...
      try {
        const rivolutModule = require('../modules/rivolut');
        const teste = await rivolutModule.testConnection(credentials);
