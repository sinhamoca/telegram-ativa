// src/modules/lumina.js - Módulo de ativação Lumina Player

const axios = require('axios');

/**
 * Configurações do Lumina Player
 * Estrutura idêntica ao DreamTV, sem CAPTCHA
 */
const LUMINA_CONFIG = {
  name: 'Lumina Player',
  baseUrl: 'https://api.luminaplayer.com',
  
  // Pacotes disponíveis
  packages: {
    YEAR: { id: 5, name: 'Anual', days: 365 },
    LIFETIME: { id: 6, name: 'Vitalício', days: 36500 }
  },
...
/**
 * Classe do ativador Lumina
 */
class LuminaActivator {
  constructor(credentials) {
    ...
    this.client = axios.create({
      baseURL: LUMINA_CONFIG.baseUrl,
      timeout: LUMINA_CONFIG.timeout,
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36',
        'Origin': 'https://reseller.luminaplayer.com',
        'Referer': 'https://reseller.luminaplayer.com/'
      }
    });
  }
  ...
  async login() {
    try {
      ...
      console.log(`[${LUMINA_CONFIG.name}] Fazendo login...`);
      ...
      if (response.status === 200 && response.data.error === false && response.data.message) {
        ...
        console.log(`[${LUMINA_CONFIG.name}] Login OK`);
      ...
      console.error(`[${LUMINA_CONFIG.name}] Erro no login:`, error.message);
  ...
  async getSaldo() {
    try {
      ...
      console.log(`[${LUMINA_CONFIG.name}] Buscando saldo...`);
      ...
      if (response.status === 200 && response.data.error === false) {
        ...
        if (reseller) {
          ...
          console.log(`[${LUMINA_CONFIG.name}] Saldo: ${credits} créditos`);
      ...
      console.error(`[${LUMINA_CONFIG.name}] Erro ao buscar saldo:`, error.message);
      return { success: false, error: error.message };
    }
  }
  ...
  extractMacAddress(rawData) {
    try {
      ...
      console.log(`[${LUMINA_CONFIG.name}] Buscando MAC em: "${cleanData}"`);
      ...
      for (const line of lines) {
        ...
        if (this.macRegex.test(trimmed)) {
          ...
          console.log(`[${LUMINA_CONFIG.name}] MAC encontrado: ${normalizedMac}`);
        ...
        if (macWithoutSeparators) {
          ...
          console.log(`[${LUMINA_CONFIG.name}] MAC sem separadores: ${formattedMac}`);
          return formattedMac;
        }
      }
      
      console.log(`[${LUMINA_CONFIG.name}] Nenhum MAC encontrado`);
      return null;
      
    } catch (error) {
      console.error(`[${LUMINA_CONFIG.name}] Erro ao extrair MAC:`, error);
      return null;
    }
  }
  ...
  async activate(macAddress, tier = 'YEAR') {
    try {
      console.log(`[${LUMINA_CONFIG.name}] Iniciando ativação...`);
      console.log(`[${LUMINA_CONFIG.name}] MAC: ${macAddress}, Tier: ${tier}`);
      ...
      const packageInfo = LUMINA_CONFIG.packages[tier] || LUMINA_CONFIG.packages.YEAR;
      const packageId = packageInfo.id;
      
      console.log(`[${LUMINA_CONFIG.name}] Ativando com pacote ${packageInfo.name} (ID: ${packageId})...`);
      ...
      console.log(`[${LUMINA_CONFIG.name}] Payload:`, JSON.stringify(payload));
      ...
      console.log(`[${LUMINA_CONFIG.name}] Resposta:`, response.status, JSON.stringify(response.data));
      ...
      if (errorMessage.toLowerCase().includes('not found') || errorMessage.toLowerCase().includes('does not exist')) {
        ...
        errorMessage = 'Créditos insuficientes na conta Lumina.';
      ...
      console.error(`[${LUMINA_CONFIG.name}] Erro na ativação:`, error.message);
      ...
      if (error.message?.includes('401') || error.message?.includes('403') || error.message?.includes('Unauthorized')) {
        console.log(`[${LUMINA_CONFIG.name}] Tentando relogin...`);
        this.accessToken = null;
        this.tokenExpiryTime = null;
